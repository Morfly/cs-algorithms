...
import sequential.math.count3SmoothNumbers
import kotlin.math.min
import kotlin.math.round


/**
 * Pratt's gap sequence for Shellsort which consists of 3-smooth numbers.
 * In number theory, a n-smooth number is an integer whose prime factors are all less or equal to n.
 *
 * @see [Smooth Numbers](https://en.wikipedia.org/wiki/Smooth_number)
 */
object PrattGapSequence {

    /**
     * Precomputed gap sequence for arrays up to 1_000_000 elements.
     */
    val get = intArrayOf(
     ...
     *
     * Note: this function generates reverse sequence of 3-smooth numbers (from greater to smaller).
     * For more information refer to [list3SmoothNumbers] which generates straight sequence
     * (from smaller to greater).
     *
     * @see [list3SmoothNumbers]
     */
    fun compute(arraySize: Int): IntArray {
        if (arraySize <= 1) return intArrayOf(1)

        var gaps = IntArray(computeSequenceSize(arraySize))
